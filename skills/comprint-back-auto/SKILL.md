...
Before engaging the backend workflow, confirm this is actually backend/FastAPI work. Do not assume — verify. This gate prevents the skill from hijacking non-backend requests in mixed-stack projects or frontend-only repositories.

### Step 1: Check project dependencies

Use Glob to find dependency files in the project:
- `**/pyproject.toml` — look for `fastapi` in `[project.dependencies]` or `[tool.poetry.dependencies]`
- `**/requirements*.txt` — look for `fastapi` on any line (may include version pins like `fastapi>=0.100.0`)
- `**/Pipfile` — look for `fastapi` in `[packages]`
- `**/setup.py` or `**/setup.cfg` — look for `fastapi` in `install_requires`

If none of these files exist or none contain `fastapi`, move to Step 2 before deciding.

### Step 2: Check for Python API files

Use Glob to search for common FastAPI project structures:
- `**/main.py`, `**/app.py`, `**/asgi.py` — entry point files
- `**/routes/**/*.py`, `**/endpoints/**/*.py`, `**/routers/**/*.py` — route modules
- `**/api/**/*.py` — API directory patterns
- `**/core/config.py`, `**/core/settings.py` — FastAPI configuration patterns

Read any matches and look for FastAPI imports (`from fastapi import FastAPI`, `from fastapi import APIRouter`, `import fastapi`). Also check for Starlette imports which indicate ASGI/FastAPI-adjacent work.

### Step 3: Decision

- **FastAPI confirmed** (found in dependencies OR in Python imports): proceed to Context Detection below. The backend workflow applies.
- **FastAPI not found but Python API files exist** (e.g., Flask, Django): exit gracefully. This skill is specifically for FastAPI projects. Say: "This appears to be a Python API project but not FastAPI. Proceeding with standard assistance."
- **Not a Python API project at all** (no Python API files, no relevant dependencies): exit gracefully with: "This doesn't appear to be a FastAPI/Python API project. Proceeding with standard assistance." Handle the user's request normally without the backend workflow.
- **Ambiguous** (e.g., monorepo with multiple frameworks): check which subdirectory the user is working in. If their recent files are in a FastAPI subdirectory, proceed. Otherwise, exit gracefully.

---

## Context Detection

Determine what the user is working on before classifying the task. This detection phase gathers signals from three sources and feeds them into Phase 1 classification. Do not skip this — the quality of classification depends on accurate context.

### Signal Source 1: Recently modified files

Use `git diff --name-only HEAD~5` and `git log --oneline -5 --diff-filter=M` to find recently changed files. Map the file types to likely work areas:

- Route/endpoint files modified → likely endpoint or API work
- Model files modified → likely database work
- Test files modified → likely testing work
- Auth/security files modified → likely auth work
- Config/settings files modified → likely project setup or configuration work
- Migration files modified → likely database schema work
- Service/repository files modified → likely business logic or refactoring work

If git is not available (new project, no commits yet), use Glob to scan the project structure and identify what exists.

### Signal Source 2: Error messages in conversation

Scan the user's message and any preceding conversation context for:

- **HTTP status codes:** 422 (validation error), 500 (server error), 404 (not found), 403 (forbidden), 401 (unauthorized), 409 (conflict)
- **Python tracebacks:** `Traceback (most recent call last)`, `File "..."`, exception class names
- **FastAPI-specific errors:** `RequestValidationError`, `HTTPException`, Pydantic validation errors, dependency injection failures
- **Database errors:** `IntegrityError`, `OperationalError`, migration failures, connection pool exhaustion
- **Test failures:** pytest output, assertion errors, fixture errors, import errors in test modules
- **CORS errors:** browser console messages about cross-origin requests, preflight failures

Each error type maps to specific task classifications and reference files — use this mapping in Phase 1.

### Signal Source 3: Request text analysis

Parse the user's request for task-signal keywords:

- **Creation signals:** "create", "add", "build", "new", "setup", "init", "implement", "make" → new work (project, endpoint, feature)
- **Fix signals:** "fix", "broken", "error", "bug", "wrong", "failing", "crash", "not working", "issue" → bug fix / debug work
- **Testing signals:** "test", "coverage", "fixture", "pytest", "mock", "assert", "spec" → testing work
- **Refactoring signals:** "refactor", "clean", "restructure", "reorganize", "simplify", "improve", "optimize" → refactoring work
- **Infrastructure signals:** "deploy", "docker", "config", "env", "settings", "migrate" → project setup or database work

Combine signals from all three sources. When signals conflict (e.g., user says "fix" but recently modified files are all test files), weight the user's explicit request highest, then error messages, then file history.

---

## Vague Input Handling

When the user's request is ambiguous, use this table to determine the correct entry point. Vague requests are the most common trigger for this auto-skill — users often describe problems loosely when they are unsure where the issue lives.

| User says | Skill detects | Action |
|-----------|--------------|--------|
| "build the backend" | Backend work, vague scope | Phase 0 Clarify — what backend? What features? Read the codebase to determine what exists and what is missing. Check for existing project structure, models, routes. |
| "fix the backend" | Backend bug, vague target | Phase 0 Clarify — read error logs, test output, recent git changes to identify the actual problem. Run `pytest --tb=short` if tests exist. Check application logs. |
| "something is not working" | Possibly backend if FastAPI project | Check if it's a backend issue by reading logs and recent test output. Look for running server errors. If backend-related, classify and proceed. If frontend, exit gracefully. |
| "the API is broken" | API issue, needs diagnosis | Load `${CLAUDE_PLUGIN_ROOT}/references/common-errors.md`, identify the specific failure mode, then enter root-cause protocol. Do not guess — reproduce the error first. |
| "getting a 500 error" | Server error, specific symptom | Load `${CLAUDE_PLUGIN_ROOT}/references/common-errors.md` + `${CLAUDE_PLUGIN_ROOT}/references/exceptions.md`. Trace the 500 to its origin in the stack trace. Check if it is an unhandled exception, a database error, or a dependency failure. |
| "add a new feature" | Possibly backend if involves API/data | Classify the feature based on what layers it touches. If it requires new endpoints, models, or services, load relevant references and proceed through the full phase process. |

**Key principle for vague inputs:** Never guess what the user means. Either determine it from the codebase (preferred) or ask the user directly (fallback). Phase 0 exists specifically for this purpose — use it.

---

## Core Philosophy

Every action you take under this skill follows four rules. No exceptions.
